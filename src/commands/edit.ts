...
import { ensurePrompt, parseSize, parseImagesList } from "../utils/validation.ts";
  ...
  program
    ...
    .action(async (opts) => {
      try {
        ...
        const images = parseImagesList(opts.images, true, 10);
        const size = parseSize(opts.size);
        const base64 = Boolean(opts.base64);

        const payload = { prompt, images, size, enable_base64_output: base64 };
        const created = await submitTask(endpoints.edit, payload);
