...
export function registerModelsSelect(program: Command) {
  program
    ...
    .action(async () => {
      ...
      if (apiKey) {
        try {
          ...
          apiChoices = apiModels.map((m) => {
            ...
            return {
              title: (model.name as string) || (model.model_id as string),
              description: `[API] ${(model.description as string) || (model.type as string)}`,
      ...
      const uniqueApiChoices = apiChoices.filter(
        (am) =>
      ...
      const rawChoices = [
        ...
        ...registryChoices,
        ...(uniqueApiChoices.length
          ? [{ title: "--- API (Live) ---", disabled: true }, ...uniqueApiChoices]
          : []),
      ];
      ...
      try {
        ...
        const newConfig: WavespeedConfig = config || { models: {} };
